<template>
  <div v-if="lookup && limbs">
    <div class="bar-detail">
      <div v-for="(count, index) in bar.counts" :key="index" :class="css.count(count, index)">
        <pose-button
    ...
    <div class="bar-editor__pose-box">
      <div v-for="(poses, limb) in lookup.by_limb" :key="limb" class="bar-editor__pose-limb">
        <div class="btn -primary" v-for="pose in poses" :key="pose.id" @click="clickPose(pose)">
          {{ pose.short }}
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    const css = {
      count: (count, index) => ['bar-detail__count', this.current_count === index && '-current'],
    }
  ...
  methods: {
    ...
    clickPose(pose) {
      ...
      if (count.includes(pose.id)) {
        ...
        const existing = count.find((id) => this.lookup.by_id[id].limb === pose.limb)
        if (existing) {
